...
import { Mail, Lock, ArrowRight, Bike, Eye, EyeOff, ChevronLeft } from 'lucide-react';
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok && data.token) {
                ...
                setError(data.detail || 'Invalid courier credentials.');
            }
        } catch (err) {
            console.error(err);
        ...
        <div className="h-screen bg-white flex font-brand select-none overflow-hidden">
            ...
            <div className="hidden lg:block w-1/2 h-full relative overflow-hidden group">
                <div className="absolute inset-0 z-0">
                    <img src={loginHero} alt="Courier Logistics" className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-[4s]" />
                ...
                <div className="relative z-10 flex flex-col h-full justify-between p-24">
                    <div className="space-y-12">
                        ...
                        <div className="space-y-8 max-w-lg">
                            <div className="inline-flex items-center gap-3 text-white/60 uppercase tracking-[0.6em] text-[10px] font-black"><div className="w-10 h-[2px] bg-brand-dark" />Courier Network</div>
                            <h1 className="text-7xl font-black text-white leading-[0.8] tracking-tighter uppercase">Courier <br /><span className="text-brand-dark italic font-outfit">Logistics.</span></h1>
                            <p className="text-xl text-white/70 font-medium leading-relaxed italic max-w-sm">Access your delivery route, earnings dashboard, and real-time fleet coordination.</p>
                        </div>
                    </div>
                    <div className="bg-white/10 backdrop-blur-xl px-10 py-5 rounded-full border border-white/10 w-fit">
                        <p className="text-[11px] font-black text-white uppercase tracking-[0.5em] italic">Fleet Management Portal</p>
                    </div>
                </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 h-full flex flex-col items-center justify-center bg-white p-6 sm:p-10 relative overflow-hidden">
                ...
                <div className="w-full max-w-lg flex flex-col justify-center">
                    <div className="mb-8 flex items-center justify-between">
                        ...
                        <Link to="/register/rider" className="text-sm font-black text-brand-dark uppercase tracking-[0.3em] border-b-4 border-brand-dark/10 pb-1 italic hover:text-accent-brown transition-all">Join Fleet</Link>
                    </div>

                    <form onSubmit={handleLogin} className="space-y-8">
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                            <div className="space-y-1">
                                <h3 className="text-5xl font-black text-accent-brown tracking-tighter uppercase leading-none">Courier Login</h3>
                                <p className="text-xs text-accent-brown/40 font-medium italic">Welcome back to the road. Ready to deliver?</p>
                            </div>

                            <div className="space-y-4">
                                <div className="group space-y-2">
                                    <label className="text-[11px] font-black text-accent-brown/30 uppercase tracking-[0.2em] pl-6 italic transition-colors group-focus-within:text-brand-dark">Account Email <span className="text-brand-dark">*</span></label>
                                ...
                                <div className="group space-y-2">
                                    <div className="flex justify-between items-center pr-6">
                                        ...
                                        <Link to="/forgot-password?role=rider" className="text-[10px] font-black text-brand-dark/40 hover:text-brand-dark uppercase tracking-widest transition-colors italic">Reset Password?</Link>
                                    </div>
                                    <div className="relative ring-1 ring-brand-dark/5 focus-within:ring-brand-dark/30 rounded-3xl transition-all shadow-inner bg-[#F7F6F2]">
                    ...
                    <div className="mt-10 space-y-8">
                        <div className="text-center pt-10 border-t border-accent-brown/5">
                            <Link to="/for-riders" className="inline-flex items-center gap-4 text-sm font-black uppercase tracking-[0.4em] text-accent-brown/30 hover:text-brand-dark transition-all group italic">
