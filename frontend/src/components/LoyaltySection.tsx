            ...
            <div className="container mx-auto px-4 xs:px-6 sm:px-8">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    ...
                    <motion.div
                        ...
                        viewport={{ once: true }}
                        transition={{ duration: 0.6 }}
                        className="bg-white rounded-[2rem] p-6 xs:p-8 md:p-10 shadow-xl shadow-accent-brown/5 border border-white"
                    >
                        <h3 className="text-xl xs:text-2xl font-black text-accent-brown tracking-tight mb-6 xs:mb-8">How to Earn Points</h3>
                        <div className="space-y-4 xs:space-y-6">
                            {earning.map((e) => (
                                <div key={e.label} className="flex items-center gap-3 xs:gap-5">
                                    <div className="w-10 h-10 xs:w-12 xs:h-12 rounded-2xl bg-accent-peach/30 flex items-center justify-center text-brand-dark shrink-0">
                                        <e.icon className="w-4 h-4 xs:w-5 xs:h-5" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="font-bold text-accent-brown text-sm xs:text-base truncate">{e.label}</p>
                                    </div>
                                    <span className="text-[8px] xs:text-[10px] font-black uppercase tracking-widest text-brand-dark bg-brand/10 px-2 xs:px-3 py-1 xs:py-1.5 rounded-full whitespace-nowrap">
                                        {e.pts}
                                    </span>
                                </div>
                            ))}
                    ...
                    <motion.div
                        ...
                        className="bg-white rounded-[2rem] p-6 xs:p-8 md:p-10 shadow-xl shadow-accent-brown/5 border border-white"
                    >
                        <h3 className="text-xl xs:text-2xl font-black text-accent-brown tracking-tight mb-6 xs:mb-8">Sample Rewards</h3>
                        <div className="grid grid-cols-2 gap-3 xs:gap-4">
                            {rewards.map((r) => (
                                <div key={r.title} className="bg-accent-peach/20 rounded-2xl p-3 xs:p-4 flex flex-col gap-2 xs:gap-3">
                                    <div className="w-8 h-8 xs:w-10 xs:h-10 bg-white rounded-xl flex items-center justify-center text-brand-dark shadow-sm shrink-0">
                                        <r.icon className="w-4 h-4 xs:w-5 xs:h-5" />
                                    </div>
                                    <p className="font-bold text-accent-brown text-xs xs:text-sm leading-tight line-clamp-2">{r.title}</p>
                                    <span className="flex items-center gap-1 text-[8px] xs:text-[10px] font-black text-brand-dark uppercase tracking-widest">
