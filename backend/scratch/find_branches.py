...
def find_branches():
    ...
    try:
        ...
        with engine.connect() as conn:
            # Find Alpha Veterinary Clinic
            query = text("SELECT id, clinic_name FROM business_profiles WHERE clinic_name ILIKE '%Alpha Veterinary%'")
            result = conn.execute(query)
            profiles = result.fetchall()
            
            if not profiles:
                print("No clinic found matching 'Alpha Veterinary'.")
                return

            for pid, p_name in profiles:
                print(f"\nClinic: {p_name} (ID: {pid})")
