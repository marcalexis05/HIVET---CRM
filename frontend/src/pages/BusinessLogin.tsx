...
const BusinessLogin = () => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        if (email === 'business@hivet.com' && password === 'business123') {
            login(email, 'business', 'Paws & Co. Partner');
            navigate('/dashboard/business');
        } else {
            // Attempt business login via backend
            try {
                const res = await fetch('http://localhost:8000/api/auth/login', { // Note: Backend may need a separate business login route in future
                    method: 'POST',
                ...
                if (res.ok) {
                    ...
                    navigate('/dashboard/business'); // Business users go to business dashboard
                } else {
                    setError('Invalid partner email or password.');
                }
            } catch (err) {
                console.error(err);
